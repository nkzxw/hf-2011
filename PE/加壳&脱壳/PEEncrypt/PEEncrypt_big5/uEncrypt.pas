...
type
  ...
  TAttachData=packed record
    ...
    _szTitlePassword:	       array[0..11]  of Char; // "請輸入密碼:"
    _szOK:		       array[0..8]   of Char; // "確定(&O)"
    _szCancel:                 array[0..8]   of Char; // "取消(&C)"
    _szWrongPassword:	       array[0..24]  of Char; // "密碼不正確,請重新輸入!"
    _szTemplate:	       array[0..29]  of Char; // "--= 你還剩下 %d 次機會 =--"
    _dwPasswordCrc32:	       DWORD;                 // 密碼的CRC32校檢碼
    _szChanceCount:	       array[0..255] of Char; // 256字節的緩衝區
...
begin
  ...
  begin
    ...
    _szMsgAbout 	       :='[ PE Encrypt ]'#13#10
                                ...
                                +'作者: Liwuyue'#13#10
                                +'郵箱: smokingroom@sina.com'#13#10
                                +'主頁: http://www.programmerlife.com'#0;

    _szClassEdit	       :='Edit'#0;
    _szClassStatic	       :='Static'#0;
    _szClassButton	       :='Button'#0;
    _szTitlePassword	       :='請輸入密碼:'#0;
    _szOK		       :='確定(&O)'#0;
    _szCancel                  :='取消(&C)'#0;
    _szWrongPassword	       :='密碼不正確,請重新輸入!'#0;
    _szTemplate 	       :='--= 你還剩下 %d 次機會 =--'#0;
    _nCount		       :=3;
  end;
end;
...
begin
  ...
  //*******在Kernel32.dll裡面查找GetProcAddress函數的線性地址
  ...
  begin
    ...
    hLibUser32                 :=_LoadLibrary(szLibUser32);   //載入user32.dll
    hLibGDI32                  :=_LoadLibrary(szLibGDI32);    //載入gdi32.dll
...
begin
  ...
  begin
    MessageBox(Handle,'打開文件失敗!',DlgCaption,MB_ICONERROR+MB_OK);
    Exit;
  end;
  try
    ...
    begin
      MessageBox(Handle,'這不是一個有效的PE文件!',DlgCaption,MB_ICONERROR+MB_OK);
      Exit;
    end;
    ...
    begin
      ...
      begin
        MessageBox(Handle,'呵呵, 您忘了嗎? 文件已經加密了!',DlgCaption,MB_ICONINFORMATION+MB_OK);
        Exit;
      end;
    ...
    //******計算新節的PointerToRawData (為512的倍數<win2000/xp下正確加載>)
    ...
    //******設置新節的Characteristics (code/data/execute/read/write/inited data/un-inited data)
