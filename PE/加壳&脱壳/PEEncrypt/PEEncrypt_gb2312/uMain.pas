...
begin
  ...
  if not FileExists(FileName) then
    ...
    MessageBox(Handle,'请输入密码!',PChar(Caption),MB_ICONERROR+MB_OK)
  else if Length(EdtRptPwd.Text)=0 then
    MessageBox(Handle,'请输入确认密码!',PChar(Caption),MB_ICONERROR+MB_OK)
  else if EdtPwd.Text<>EdtRptPwd.Text then
    MessageBox(Handle,'确认密码与密码不一致!',PChar(Caption),MB_ICONERROR+MB_OK)
  else
    Result:=True;
end;
