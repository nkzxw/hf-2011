...
/*
	Читаем секции из файл??грузим их по соответствующи?виртуальны?адреса?
*/
{
...
*/
{
	...
	for( ; impdesc->Name ; impdesc++ )
	{
		...
		// откуда брат?функци?- из FirstThunk ил?из OriginalFirstThunk
		DWORD RvaOfThunks = impdesc->FirstThunk;

		// Импорт забинден?
		if( impdesc->TimeDateStamp == -1 )
		{
			// Да, чекаем на валидность тупо?hgkgjhgkhkjhkhгншгншнгроверкой первой функци?
			DWORD *Func;
			...
			// адреса не совпадаю? пробинденный импорт надо перестроит?
			else
			{
				printf("FAILED [needs rebuilding]\n");
				RvaOfThunks	= impdesc->OriginalFirstThunk; // для ребиндинга брат?функци?из OriginalFirstThunk
			}
		}
		else
			printf("NOT BOUND\n");
		...
		for( PIMAGE_THUNK_DATA Thunk = (PIMAGE_THUNK_DATA)RVATOVA(RvaOfThunks, base); Thunk->u1.Ordinal ; Thunk++ )
		{
			...
			// не важн?откуда читали, записываем вс?равн??FirstThunk
...
/*
	Выставит?атрибуты секций содержащим их страница?
*/
{
	PIMAGE_FILE_HEADER		pfh;
	PIMAGE_OPTIONAL_HEADER	poh;
...
/*
	Загружае?обра??пути path по адресу base.
	Если base=0, использует? поле IMAGE_OPTIONAL_HEADER.ImageBase
*/
{
	HANDLE hFile;
	PIMAGE_FILE_HEADER		pfh;
...
//		SetLastError(RtlNtStatusToDosError(STATUS_INVALID_IMAGE_NOT_MZ));
	...
	//
	// Резервируе?пространство по?обра?
	//
...
_repeat:
	...
	//
	// Вс?удачно, закрывае?файл ?возвращаем базу
	//
	// Важн? Файл не гото??запуск? Чтоб?запустит?этот обра?
	//  нужн?ещ?скорректироват?поле PEB.ImageBaseAddress ?PEB,
	//  инач?не буду?работать функци?Win32 для работы ?ресурсам?
	//
...
/*
	Подготовит?PEB процесса ?запуск?
*/
{
...
//	peb->ImageBaseAddress = base;
	...
	// Добавляем модуль ?PEB.Ldr.In...OrderModuleList
