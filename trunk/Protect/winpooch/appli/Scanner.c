...
BOOL Scanner_Init ()
{
  ...
  // -> NULL, internal antirivus
  if( ! szConfigScanner )
    {
      if( _Scanner_LibClamav_Configure (&g_data.libclamavconf) )
...
BOOL Scanner_SetScanner (UINT nScanner)
{
  ...
  Config_SaveString (CFGSTR_ANTIVIRUS) ;
...
UINT Scanner_ScanFile (LPCTSTR szPath, LPTSTR szOutput, UINT nOutputMax, BOOL bBackground)
{
  TRACE_INFO ("File = %s\n", szPath) ;
  ...
  switch( g_data.nScanner )
    ...
    case SCANNER_CLAMWIN:
      return _Scanner_ClamWin_ScanFile (&g_data.clamwinconf, szPath, szOutput, nOutputMax, 
					bBackground ? IDLE_PRIORITY_CLASS : HIGH_PRIORITY_CLASS) ;
				...
				DWORD nPriorityClass) 
{
  ...
  wsprintf (szCmdLine, TEXT("\"%s\" -d \"%s\" --max-space=%u \"%s\""), 
	    pConf->szScanner, pConf->szDatabase, MAX_SPACE/1024, szFile) ;
  ...
  return
    dwExitCode==0 || dwExitCode==54 ? SCAN_NO_VIRUS :
    dwExitCode==1 ? SCAN_VIRUS :
    SCAN_FAILED ;
}
...
{    
  ...
  conf->options = CL_SCAN_MAIL | CL_SCAN_OLE2 | CL_SCAN_HTML | CL_SCAN_PE | CL_SCAN_ALGORITHMIC ;
  ...
  conf->limits.maxfilesize = (unsigned long int) MAX_SPACE;
...
BOOL _Scanner_LibClamav_LoadDatabase (LIBCLAMAVCONF* conf)
{
  ...
  pNewEngine = NULL ;
  iResult = cl_load (szDbDirectory, &pNewEngine, &nSignCount, CL_DB_STDOPT);
  if( iResult!=0 )
    {
				  ...
				  LPTSTR szOutput, UINT nOutputMax)
{
  ...
  return
    ...
    iResultCode==CL_VIRUS ? SCAN_VIRUS :
    SCAN_FAILED ;
