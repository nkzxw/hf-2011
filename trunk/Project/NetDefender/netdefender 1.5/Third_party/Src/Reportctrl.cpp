...
int CReportCtrl::InsertItem(UINT	nMask,
							int		nItem,
							LPCTSTR lpItem,
							UINT	nState,
							UINT	nStateMask,
...
BOOL CReportCtrl::SetItemText(int nItem, int nSubItem, const COleDateTime &dateTime, DWORD dwFlags)
{
	return
		(
			SetItemText(nItem,
						nSubItem,
						dateTime.GetStatus() == COleDateTime :: valid ? dateTime.Format(dwFlags) : _T(""))
		);
}
...
BOOL CReportCtrl::_IsEditVisible() const
{
	return
		(
			m_bAllowEdit &&
			m_ptEditting.x >= 0 &&
			m_ptEditting.x < CListCtrl :: GetItemCount() &&
			m_ptEditting.y >= 0 &&
...
{
	if(!_IsValidIndex(nIndex) || m_nChkStyle == RC_CHKBOX_NONE || (bBefore && bAfter) || (!bBefore && !bAfter))
	{
		return(FALSE);
	}
...
BOOL CReportCtrl::StartEdit(int nItem, int nSubItem)
{
	...
	{
		if(m_pWndEdit->GetSafeHwnd() == NULL && !m_pWndEdit->Create(ES_AUTOHSCROLL | ES_NOHIDESEL | WS_CHILD | WS_BORDER | FMT,
...
BOOL CReportCtrl::SetItem(int	  nItem,
						  int	  nSubItem,
						  UINT	  nMask,
						  LPCTSTR lpszItem,
						  int	  nImage,
