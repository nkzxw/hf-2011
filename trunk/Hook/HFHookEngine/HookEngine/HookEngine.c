...
#include "../disasm/include/distorm.h"
...
JMP_REL MakeRelativeJump (PVOID pFromAddr, PVOID pToAddr)
{
	JMP_REL JumpREL;
	...
	JumpREL.operand = (ULONG_PTR)pToAddr - ((ULONG_PTR)pFromAddr + sizeof (JMP_REL));
	
	return JumpREL;
}

JMP_ABS MakeAbstractJump(pToAddr)
{
	JMP_ABS JumpABS;
	...
	JumpABS.operand = (ULONG_PTR)pToAddr;
