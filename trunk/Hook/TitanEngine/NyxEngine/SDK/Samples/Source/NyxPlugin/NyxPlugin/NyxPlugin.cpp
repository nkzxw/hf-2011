// NyxPlugin.cpp : Defines the exported functions for the DLL application.
//
...
HANDLE __stdcall OpenArchive(tOpenArchiveData* ArchiveData){
	...
	nyxOpenFileHandle = NyxOpenArchive(ArchiveData->ArcName, GENERIC_READ);
	if(nyxOpenFileHandle != NULL){
		if(NyxIdentifyArchive(nyxOpenFileHandle)){
			return((HANDLE)nyxOpenFileHandle);
		}else{
...
HANDLE __stdcall OpenArchiveW(tOpenArchiveDataW* ArchiveData){
	...
	nyxOpenFileHandle = NyxOpenArchiveW(ArchiveData->ArcName, GENERIC_READ);
	if(nyxOpenFileHandle != NULL){
		if(NyxIdentifyArchive(nyxOpenFileHandle)){
			return((HANDLE)nyxOpenFileHandle);
		}else{
...
int __stdcall ReadHeader(HANDLE hArcData, tHeaderData* HeaderData){
	...
	if(NyxGetNextFile((long)hArcData, &nyxFileInfo)){
		lstrcpyA(&HeaderData->FileName[0], &nyxFileInfo.nyxFileName[0]);
		HeaderData->FileAttr = nyxFileInfo.nyxFileAttributes; 
		HeaderData->FileTime = nyxFileInfo.nyxFileCreationTime;
		HeaderData->PackSize = nyxFileInfo.nyxCompressedSize;
		HeaderData->UnpSize = nyxFileInfo.nyxUncompressedSize;
		HeaderData->FileCRC = nyxFileInfo.nyxFileChecksum;
		return(NULL);
	}else{
...
int __stdcall ReadHeaderEx(int hArcData, tHeaderDataEx* HeaderDataEx){
	...
	if(NyxGetNextFile((long)hArcData, &nyxFileInfo)){
		lstrcpyA(&HeaderDataEx->FileName[0], &nyxFileInfo.nyxFileName[0]);
		HeaderDataEx->FileAttr = nyxFileInfo.nyxFileAttributes; 
		HeaderDataEx->FileTime = nyxFileInfo.nyxFileCreationTime;
		HeaderDataEx->PackSize = nyxFileInfo.nyxCompressedSize;
		HeaderDataEx->UnpSize = nyxFileInfo.nyxUncompressedSize;
		HeaderDataEx->FileCRC = nyxFileInfo.nyxFileChecksum;
		return(NULL);
	}else{
		return(E_END_ARCHIVE);
	}
...
int __stdcall ProcessFile(HANDLE hArcData, int Operation, char* DestPath, char* DestName){
	...
	if(Operation == PK_TEST || Operation == PK_SKIP){
		return(NULL);
	}else{
		if(DestPath == NULL){
			lstrcpyA(szFileName, DestName);
		}else{
			lstrcpyA(szFileName, DestPath);
			lstrcatA(szFileName, DestName);
		}
		if(NyxRecoverFile((long)hArcData, NYX_SELECT_PREVIOUS_FILE, szFileName) != NYX_RECOVER_SUCCESS){
			return(E_BAD_DATA);
		}else{
			return(NULL);
		}
...
int __stdcall ProcessFileW(int hArcData, int Operation, WCHAR* DestPath, WCHAR* DestName){
	...
	if(Operation == PK_TEST || Operation == PK_SKIP){
		...
		if(DestPath == NULL){
			lstrcpyW(szFileName, DestName);
		}else{
			lstrcpyW(szFileName, DestPath);
			lstrcatW(szFileName, DestName);
		}
		if(NyxRecoverFileW((long)hArcData, NYX_SELECT_PREVIOUS_FILE, szFileName) != NYX_RECOVER_SUCCESS){
			return(E_BAD_DATA);
		}else{
			return(NULL);
		}
...
int __stdcall GetPackerCaps(){
  return PK_CAPS_NEW | PK_CAPS_MODIFY | PK_CAPS_BY_CONTENT | PK_CAPS_MULTIPLE;
}
bool __stdcall CanYouHandleThisFile(char* filename){

	long nyxOpenFileHandle = NULL;
	
	nyxOpenFileHandle = NyxOpenArchive(filename, GENERIC_READ);
	if(nyxOpenFileHandle != NULL){
		if(NyxIdentifyArchive(nyxOpenFileHandle)){
			NyxCloseArchive(nyxOpenFileHandle);
			return(true);
...
bool __stdcall CanYouHandleThisFileW(WCHAR* filename){
	...
	nyxOpenFileHandle = NyxOpenArchiveW(filename, GENERIC_READ);
	if(nyxOpenFileHandle != NULL){
		if(NyxIdentifyArchive(nyxOpenFileHandle)){
			NyxCloseArchive(nyxOpenFileHandle);
			return(true);
