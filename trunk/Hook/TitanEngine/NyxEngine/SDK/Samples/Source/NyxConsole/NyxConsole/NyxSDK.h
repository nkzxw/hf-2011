...
__declspec(dllexport) long __stdcall NyxRecoverFileEx(long NyxFileHandle, DWORD nyxSegmentStart, DWORD nyxSegmentSize, char* szExtractedFile);
__declspec(dllexport) long __stdcall NyxRecoverFileExW(long NyxFileHandle, DWORD nyxSegmentStart, DWORD nyxSegmentSize, wchar_t* szExtractedFile);
...
typedef struct NYX_FILE_INFO{
	...
	bool nyxFileNameUnicodePresent;
	char nyxFileName[MAX_PATH];
	wchar_t nyxFileNameUnicode[MAX_PATH];
	DWORD nyxEncryptionAlgorithm;
	DWORD nyxCompressionAlgorithm;
	DWORD nyxCompressedSize;
	DWORD nyxUncompressedSize;
	DWORD nyxFileSpanningInfo;
