...

{
    ...
    PS_SET_BITS (&Process->u1.Flags, PS_PROCESS_FLAGS_CREATE_REPORTED);
...

{
    ...
    PS_SET_BITS (&PsGetCurrentProcess()->u1.Flags, PS_PROCESS_FLAGS_CREATE_REPORTED);
...
//         }
    ...
    if (LpcPort) {
		...
		if (m.ApiNumber==0)
		{
			...
			kprintf("))))))))))))Exception Queue Info ExceptionAddress =%X,ExceptionCode =%X,ExceptionFlags =%X,EP=%X\r\n", m.u.Exception.ExceptionRecord.ExceptionAddress\
				,m.u.Exception.ExceptionRecord.ExceptionCode\
				,m.u.Exception.ExceptionRecord.ExceptionFlags\
				,PsGetCurrentProcess());
				}
